...
pub fn ansi_to_html(bytes: &[u8]) -> String {
    ...
    ansi_vec.iter().for_each(|ansi_seq| {
            ...
            Output::Escape(seq) => {
                match seq {
                    AnsiSequence::SetGraphicsMode(d) => {
                        d.iter()
                            .for_each(|s| graphics_mode_class(s, &mut active_styles));
                    }
        ...
        if !text.is_empty() {
            ...
            let eols = text
                .matches("\r\n")
                .map(|_| "<div style=\"flex-basis:100%;height:0;\"></div>")
                .collect::<Vec<&str>>()
                .join("");
